...
function TopMovies() {
    ...
    const [getMoviesTop, setGetMoviesTop] = useState()
    ...
    const GetTopRated = async() => {
        const temp = await fetch(LINK + '/movie/top_rated?api_key=' + API_KEY)
        const data = await temp.json()

        setGetMoviesTop(data.results.slice(0, 5))
    }
         ...
         {getMoviesTop? 
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='font-bold py-5'> Top Rated </h1> <button className='text-sm border-rose-800 border rounded-lg hover:bg-rose-900' onClick={navigateToPopular("top_rated")}> View All </button>
                </div>
                    <div className='grid grid-cols-2 place-content-center gap-3 md:grid-cols-3 lg:grid-cols-5'>
                        {getMoviesTop.map(data => (
