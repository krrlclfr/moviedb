...
function UpComing() {
    ...
    const [getMoviesUpcoming, setGetMoviesUpcoming] = useState()
    ...
    const GetUpcoming = async() => {
        const temp = await fetch(LINK + '/movie/upcoming?api_key=' + API_KEY)
        const data = await temp.json()

        setGetMoviesUpcoming(data.results.slice(0, 5))
    }
        ...
        {getMoviesUpcoming? 
            <>
                <div className='flex justify-between items-center'>
                    <h1 className='font-bold py-5'> Upcoming </h1> <button className='text-sm border-rose-800 border rounded-lg hover:bg-rose-900' onClick={navigateToPopular("upcoming")}> View All </button>
                </div>
            
                <div className='grid grid-cols-2 place-content-center gap-3 md:grid-cols-3 lg:grid-cols-5'>
                    {getMoviesUpcoming.map(data => (
